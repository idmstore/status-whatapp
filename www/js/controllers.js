angular.module("status_whatapp.controllers", [])
...
.controller("indexCtrl", function($ionicConfig,$scope,$rootScope,$state,$location,$ionicScrollDelegate,$ionicListDelegate,$http,$httpParamSerializer,$stateParams,$timeout,$interval,$ionicLoading,$ionicPopup,$ionicPopover,$ionicActionSheet,$ionicSlideBoxDelegate,$ionicHistory,ionicMaterialInk,ionicMaterialMotion,$window,$ionicModal,base64,md5,$document,$sce,$ionicGesture,$translate,tmhDynamicLocale){
	...
	modal_language += "<ion-radio icon=\"icon ion-android-radio-button-on\" ng-model=\"language_option\" ng-value=\"'id-id'\" ng-click=\"tryChangeLanguage('id-id')\">Indonesian - Bahasa Indonesia</ion-radio>";
	...
	popover_template += "			<a  class=\"item dark-ink\" ng-href=\"#/status_whatapp/about_us\" ng-click=\"popover.hide()\">";
...
// TODO: quoteCtrl --|-- 
.controller("quoteCtrl", function($ionicConfig,$scope,$rootScope,$state,$location,$ionicScrollDelegate,$ionicListDelegate,$http,$httpParamSerializer,$stateParams,$timeout,$interval,$ionicLoading,$ionicPopup,$ionicPopover,$ionicActionSheet,$ionicSlideBoxDelegate,$ionicHistory,ionicMaterialInk,ionicMaterialMotion,$window,$ionicModal,base64,md5,$document,$sce,$ionicGesture,$translate,tmhDynamicLocale){
	...
	$rootScope.grid128 = parseInt($rootScope.ionWidth / 128) ;
	$rootScope.grid256 = parseInt($rootScope.ionWidth / 256) ;
	$rootScope.last_edit = "table (quote)" ;
	$scope.$on("$ionicView.afterEnter", function (){
		var page_id = $state.current.name ;
		$rootScope.page_id = page_id.replace(".","-") ;
	});
	...
	// TODO: quoteCtrl --|-- $scope.scrollTop
	$rootScope.scrollTop = function(){
		$timeout(function(){
			$ionicScrollDelegate.$getByHandle("top").scrollTop();
		},100);
	};
	// TODO: quoteCtrl --|-- $scope.openURL
	// open external browser 
	$scope.openURL = function($url){
		window.open($url,"_system","location=yes");
	};
	// TODO: quoteCtrl --|-- $scope.openAppBrowser
	// open AppBrowser
	$scope.openAppBrowser = function($url){
		var appBrowser = window.open($url,"_blank","hardwareback=Done");
		appBrowser.addEventListener("loadstart",function(){
	...
	// TODO: quoteCtrl --|-- $scope.openWebView
	// open WebView
	$scope.openWebView = function($url){
		var appWebview = window.open($url,"_blank","location=no");
		appWebview.addEventListener("loadstart",function(){
	...
	// TODO: quoteCtrl --|-- $scope.toggleGroup
	$scope.toggleGroup = function(group) {
		if ($scope.isGroupShown(group)) {
			$scope.shownGroup = null;
		} else {
	...
	// TODO: quoteCtrl --|-- $scope.showAuthentication
	...
	// TODO: quoteCtrl --|-- $scope.splitArray
	...
	// TODO: quoteCtrl --|-- $scope.addToVirtual
	$scope.addToDbVirtual = function(newItem){
		var is_already_exist = false ;
		// animation loading 
		$ionicLoading.show();
		var virtual_items = []; 
		localforage.getItem("quote_bookmark", function(err,dbVirtual){
			if(dbVirtual === null){
				virtual_items = [];
			}else{
				try{
			...
			localforage.setItem("quote_bookmark",JSON.stringify(virtual_items));
			$timeout(function(){
				$ionicLoading.hide();
			},200);
		}).catch(function(err){
	...
	// TODO: quoteCtrl --|-- $scope.fetchURL
	$scope.fetchURL = "http://demo.ihsana.net/other/index.php";
	// TODO: quoteCtrl --|-- $scope.fetchURLp
	$scope.fetchURLp = "http://demo.ihsana.net/other/index.php?callback=JSON_CALLBACK";
	// TODO: quoteCtrl --|-- $scope.hashURL
	...
	localforage.getItem("data_quotes_" + $scope.hashURL, function(err, get_quotes){
		if(get_quotes === null){
			data_quotes =[];
		}else{
			data_quotes = JSON.parse(get_quotes);
			$scope.data_quotes =JSON.parse( get_quotes);
			$scope.quotes = [];
			for(lastPush = 0; lastPush < 50; lastPush++) {
				if (angular.isObject(data_quotes[lastPush])){
					$scope.quotes.push(data_quotes[lastPush]);
				};
			}
			$timeout(function() {
				$ionicLoading.hide();
	...
	if(data_quotes.length === 0 ){
		$timeout(function() {
			...
			$http.get(url_request,http_header).then(function(response) {
				data_quotes = response.data;
				console.log("%cSuccessfully","color:blue;font-size:18px");
				console.dir(data_quotes);
				$scope.data_quotes = response.data;
				// TODO: quoteCtrl --|---------- set:localforage
				localforage.setItem("data_quotes_" + $scope.hashURL, JSON.stringify(data_quotes));
				$scope.quotes = [];
				for(lastPush = 0; lastPush < 50; lastPush++) {
					if (angular.isObject(data_quotes[lastPush])){
						$scope.quotes.push(data_quotes[lastPush]);
				...
				$timeout(function() {
					...
					if(response.status ===401){
						// TODO: quoteCtrl --|------------ error:Unauthorized
						$scope.showAuthentication();
					}else{
						// TODO: quoteCtrl --|------------ error:Message
	...
	// TODO: quoteCtrl --|-- $scope.doRefresh
	...
	// TODO: quoteCtrl --|-- controller_by_user
	// controller by user 
	function controller_by_user(){
		try {
...
$ionicConfig.backButton.text("");			
			...
			console.log("%cerror: %cPage: `quote` and field: `Custom Controller`","color:blue;font-size:18px","color:red;font-size:18px");
			console.dir(e);
		}
	}
	$scope.rating = {};
...
// TODO: quote_bookmarkCtrl --|-- 
.controller("quote_bookmarkCtrl", function($ionicConfig,$scope,$rootScope,$state,$location,$ionicScrollDelegate,$ionicListDelegate,$http,$httpParamSerializer,$stateParams,$timeout,$interval,$ionicLoading,$ionicPopup,$ionicPopover,$ionicActionSheet,$ionicSlideBoxDelegate,$ionicHistory,ionicMaterialInk,ionicMaterialMotion,$window,$ionicModal,base64,md5,$document,$sce,$ionicGesture,$translate,tmhDynamicLocale){
	...
	// TODO: quote_bookmarkCtrl --|-- $scope.scrollTop
	$rootScope.scrollTop = function(){
		$timeout(function(){
			$ionicScrollDelegate.$getByHandle("top").scrollTop();
		},100);
	};
	// TODO: quote_bookmarkCtrl --|-- $scope.openURL
	// open external browser 
	$scope.openURL = function($url){
		window.open($url,"_system","location=yes");
	};
	// TODO: quote_bookmarkCtrl --|-- $scope.openAppBrowser
	// open AppBrowser
	$scope.openAppBrowser = function($url){
		var appBrowser = window.open($url,"_blank","hardwareback=Done");
		appBrowser.addEventListener("loadstart",function(){
	...
	// TODO: quote_bookmarkCtrl --|-- $scope.openWebView
	// open WebView
	$scope.openWebView = function($url){
		var appWebview = window.open($url,"_blank","location=no");
		appWebview.addEventListener("loadstart",function(){
	...
	// TODO: quote_bookmarkCtrl --|-- $scope.toggleGroup
	$scope.toggleGroup = function(group) {
		if ($scope.isGroupShown(group)) {
			$scope.shownGroup = null;
		} else {
	...
	// TODO: quote_bookmarkCtrl --|-- $scope.redirect
	// redirect
	$scope.redirect = function($url){
		$window.location.href = $url;
	};
	...
	// TODO: quote_bookmarkCtrl --|-- Database for Cart/Bookmark
	// TODO: quote_bookmarkCtrl --|-- $scope.loadDbVirtualQuote
	 
	$scope.loadDbVirtualQuote = function(){
		$ionicLoading.show();
		//quote_bookmark
		$scope.quote_bookmark = []; 
		localforage.getItem("quote_bookmark", function(err,dbVirtual){
			if(dbVirtual === null){
				$scope.quote_bookmark = []; 
			}else{
				try{
					$scope.quote_bookmark = JSON.parse(dbVirtual); 
				}catch (e){
					$scope.quote_bookmark = []; 
				}
			}
	...
	$scope.$on("$ionicView.enter", function (){
		$scope.loadDbVirtualQuote();
	});
	// TODO: quote_bookmarkCtrl --|-- $scope.clearDbVirtualQuote
	$scope.clearDbVirtualQuote = function(){
		localforage.setItem("quote_bookmark",[]);
		$timeout(function(){
			$scope.loadDbVirtualQuote();
		},200);
	};
	// TODO: quote_bookmarkCtrl --|-- $scope.removeDbVirtualQuote
	$scope.removeDbVirtualQuote = function(itemId){
		var virtual_items = [];
		var last_items = $scope.quote_bookmark;
		for(var z=0;z<last_items.length;z++){
	...
	// TODO: quote_bookmarkCtrl --|-- controller_by_user
	// controller by user 
	function controller_by_user(){
		try {
...
$ionicConfig.backButton.text("");			
			...
			console.log("%cerror: %cPage: `quote_bookmark` and field: `Custom Controller`","color:blue;font-size:18px","color:red;font-size:18px");
			console.dir(e);
		}
	}
	$scope.rating = {};
...
// TODO: quote_singlesCtrl --|-- 
.controller("quote_singlesCtrl", function($ionicConfig,$scope,$rootScope,$state,$location,$ionicScrollDelegate,$ionicListDelegate,$http,$httpParamSerializer,$stateParams,$timeout,$interval,$ionicLoading,$ionicPopup,$ionicPopover,$ionicActionSheet,$ionicSlideBoxDelegate,$ionicHistory,ionicMaterialInk,ionicMaterialMotion,$window,$ionicModal,base64,md5,$document,$sce,$ionicGesture,$translate,tmhDynamicLocale){
	...
	// TODO: quote_singlesCtrl --|-- $scope.scrollTop
	$rootScope.scrollTop = function(){
		$timeout(function(){
			$ionicScrollDelegate.$getByHandle("top").scrollTop();
		},100);
	};
	// TODO: quote_singlesCtrl --|-- $scope.openURL
	// open external browser 
	$scope.openURL = function($url){
		window.open($url,"_system","location=yes");
	};
	// TODO: quote_singlesCtrl --|-- $scope.openAppBrowser
	// open AppBrowser
	$scope.openAppBrowser = function($url){
		var appBrowser = window.open($url,"_blank","hardwareback=Done");
		appBrowser.addEventListener("loadstart",function(){
	...
	// TODO: quote_singlesCtrl --|-- $scope.openWebView
	// open WebView
	$scope.openWebView = function($url){
		var appWebview = window.open($url,"_blank","location=no");
		appWebview.addEventListener("loadstart",function(){
	...
	// TODO: quote_singlesCtrl --|-- $scope.toggleGroup
	$scope.toggleGroup = function(group) {
		if ($scope.isGroupShown(group)) {
			$scope.shownGroup = null;
		} else {
	...
	// TODO: quote_singlesCtrl --|-- $scope.redirect
	// redirect
	$scope.redirect = function($url){
		$window.location.href = $url;
	};
	...
	// TODO: quote_singlesCtrl --|-- $scope.showAuthentication
	$scope.showAuthentication  = function(){
		var authPopup = $ionicPopup.show({
			template: ' This page required login',
			title: "Authorization",
	...
	// TODO: quote_singlesCtrl --|-- $scope.addToVirtual(); //data single
	$scope.addToDbVirtual = function(newItem){
		var is_already_exist = false ;
		// animation loading 
		$ionicLoading.show();
	...
	// TODO: quote_singlesCtrl --|-- $scope.fetchURL
	$scope.fetchURL = "http://demo.ihsana.net/other/index.php";
	// TODO: quote_singlesCtrl --|-- $scope.fetchURLp
	$scope.fetchURLp = "http://demo.ihsana.net/other/index.php?callback=JSON_CALLBACK";
	// TODO: quote_singlesCtrl --|-- $scope.hashURL
	$scope.hashURL = md5.createHash($scope.fetchURL);
	
	var current_item = [];
	localforage.getItem("data_quotes_" + $scope.hashURL, function(err, get_datas){
		if(get_datas === null){
			current_item = [];
		}else{
			if(get_datas !== null){
	...
	if( current_item.length === 0 ){
		...
		// TODO: quote_singlesCtrl --|-- $http.get
		$http.get($scope.fetchURL,http_header).then(function(response) {
			// Get data single
			var datas = response.data;
			// TODO: quote_singlesCtrl --|---------- set:localforage
			localforage.setItem("data_quotes_"+ $scope.hashURL,JSON.stringify(datas));
			for (var i = 0; i < datas.length; i++) {
				if((datas[i].id ===  parseInt(itemID)) || (datas[i].id === itemID.toString())) {
					current_item = datas[i] ;
				}
		...
		// TODO: quote_singlesCtrl --|-- $scope.doRefresh
	$scope.doRefresh = function(){
		// Retrieving data
		var itemID = $stateParams.id;
		var current_item = [];
		...
		$http.get($scope.fetchURL,http_header).then(function(response) {
			...
			// Error message
		// TODO: quote_singlesCtrl --|---------- $http.jsonp
	...
	// TODO: quote_singlesCtrl --|-- controller_by_user
	// controller by user 
	function controller_by_user(){
		try {
...
$ionicConfig.backButton.text("");			
			...
			console.log("%cerror: %cPage: `quote_singles` and field: `Custom Controller`","color:blue;font-size:18px","color:red;font-size:18px");
			console.dir(e);
		}
	}
	$scope.rating = {};
